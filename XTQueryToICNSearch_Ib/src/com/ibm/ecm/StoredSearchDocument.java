...
public class StoredSearchDocument {
	...
	public StoredSearchDocument(Connection con, String AdvanceSearchName, String objecstore, String userOwner,
			String XMLfile, String JSONfile, String AdminUser, String AdminGroup, Logger logger) {

		this.conn = con;
		this.AdvanceSearch = AdvanceSearchName;
		this.objectStore = objecstore;
		this.userOwner = userOwner;
		this.XMLFile = XMLfile;
		this.JSONFile = JSONfile;
		this.AdminUser = AdminUser;
		this.AdminGroup = AdminGroup;
		this.logger = Logger.getLogger(StoredSearchDocument.class.getName());
	...
	public void createStoredSearch() {
		...
		try {
			dom = Factory.Domain.fetchInstance(this.conn, null, null);

			ObjectStore ojbStore = Factory.ObjectStore.getInstance(dom, this.objectStore);
			...
			Document doc = Factory.Document.createInstance(ojbStore, "StoredSearch");
			doc.getProperties().putValue("DocumentTitle", this.AdvanceSearch);
			doc.getProperties().putValue("SearchType", 2);
			doc.getProperties().putValue("SearchingObjectStores", this.objectStore);
			...
			doc.getProperties().putValue("Description", this.AdvanceSearch);
